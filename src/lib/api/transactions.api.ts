...
export const getAllTransactions = async (action: string) => {
  const paramsValue = action === 'All' ? { undefined } : { action };
  const res = await api.get('/transactions', { params: paramsValue });
