...
const DashboardStockChart = ({ points, range = '', onRangeChange }: Props) => {
  const data = useMemo(
    () =>
      points.map((p) => ({
      ...
      {data.length > 0 ? (
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
            <defs>
              <linearGradient id="dashboardStockChartFill" x1="0" y1="0" x2="0" y2="1">
            ...
            <Area
              ...
              stroke="var(--primary)"
              strokeWidth={2}
              fill="url(#dashboardStockChartFill)"
              dot={data.length === 1}
              activeDot={{ r: 4 }}
            />
          </AreaChart>
